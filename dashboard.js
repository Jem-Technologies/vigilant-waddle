...
function getAllTimezones() {
  ...
  async function renderAdminUsers() {
    ...
    // ===== Admin Users: modal behavior (old-script compatible, with Groups & Permissions) =====

    // Shims/utilities to match your older helpers
    ...
    // allow either old id (#btnSaveUser) or new id (#createBtn)
    const btnSave   = document.getElementById('btnSaveUser') || document.getElementById('createBtn');
    ...
    // Inline multi-select roots
    const msGroupsRoot = document.getElementById('ufGroupsMS');
    const msPermsRoot  = document.getElementById('ufPermsMS');

    // New-user creds modal (renamed to avoid id clash)
    const credsDlg = document.getElementById('userCredsModal') || document.getElementById('userModal'); // fallback if you didn’t rename
    ...
    async function fetchJSON(url, opts = {}) {
      ...
      if (!res.ok) {
      const msg = data?.error || data?.detail || data?.message || text?.slice(0, 400) || `HTTP ${res.status}`;
      throw new Error(msg);
      }
      return data ?? {};
    }

    // Local state (kept minimal to mirror your old code)
    const state = { users: [] };

    // Bind once per route visit
    if (card && !card.dataset.bound) {

      // Open / Close behavior just like the old flow
      on(btnAdd, 'click', () => openForm());
      on(btnCancel, 'click', () => closeForm());
      on(btnGen, 'click',  () => { fieldPass.value = generatePassword(); fieldPass.focus(); });

      // Save = create (server-backed now)
      on(btnSave, 'click', async (e) => {
      e.preventDefault();
      const payload = collectForm();
      if (!payload.name || !payload.email) { toast('Name and Email are required'); return; }
      ...
      try {
        ...
        // append to local state for UI continuity (optional)
        if (res?.user) {
        state.users.unshift({
          id: res.user.id,
          name: res.user.name,
          ...
          role: res.user.role,
          privileges: payload.permission_ids || [],
          password: payload.password || '',  // temp only
          createdAt: Date.now()
        });
        }

        toast(`new user ${payload.name} added`);
        if (res?.user) openUserModal(res.user, payload.password || '');

        // Allow your existing refresh hook to repaint table
        if (typeof renderAdminUsers === 'function') {
        renderAdminUsers();
        } else if (typeof window.refreshUsers === 'function') {
        window.refreshUsers();
        }
      ...
      // Row actions on the table (edit/disable placeholders as before)
      const tbody = document.querySelector('#usersTable tbody');
      ...
      // “New user added” modal actions (download/email/edit)
      on($('#umClose'),    'click', () => safeClose(credsDlg));
    ...
    function openForm(u = null) {
      ...
      // set default permission checkbox selection (inline legacy) remains untouched
      // Now handle multi-selects: groups & permissions
      msSetSelected(msGroupsRoot, (u && u.group_ids) || []);
      msSetSelected(msPermsRoot,  (u && u.permission_ids) || []);

      // If nothing to select, ensure counts start at 0
      msUpdateCount(msGroupsRoot);
      msUpdateCount(msPermsRoot);

      // focus first field
      setTimeout(() => fieldName?.focus(), 0);
      } else {
      // Fallback: inline visibility (non-dialog)
      form.hidden = false;
      form.dataset.editing = u?.id || '';
      fieldName.value  = u?.name  || '';
      fieldEmail.value = u?.email || '';
    ...
    function closeForm() {
      ...
      // clear multi-select state & counts
      msSetSelected(msGroupsRoot, []);
      msSetSelected(msPermsRoot, []);
      msUpdateCount(msGroupsRoot);
      msUpdateCount(msPermsRoot);

      // Close dialog properly (prevents aria-hidden/focus warnings)
      const dlg = document.getElementById('addUserModal');
    ...
    function collectForm() {
      ...
      const group_ids = msGetSelected(msGroupsRoot);
      const permission_ids = msGetSelected(msPermsRoot);
      ...
      role,
      group_ids,
      // Only send permission_ids for explicit Custom role (prevents accidental custom roles)
      permission_ids: role === 'Custom' ? permission_ids : [],
      password: password || generatePassword()
      };
    }

    // ==== Multi-select helpers (inline dropdown .ms pattern you already use) ====
    function msGetSelected(root) {
      if (!root) return [];
      return Array.from(root.querySelectorAll('.ms-menu input[type="checkbox"]:checked')).map(cb => cb.value);
    }
    function msSetSelected(root, ids) {
      if (!root) return;
      const want = new Set((ids || []).map(String));
      root.querySelectorAll('.ms-menu input[type="checkbox"]').forEach(cb => {
      cb.checked = want.has(String(cb.value));
      });
    }
    function msUpdateCount(root) {
      if (!root) return;
      const count = root.querySelectorAll('.ms-menu input[type="checkbox"]:checked').length;
      const out = root.querySelector('.count span');
      if (out) out.textContent = String(count);
    }

    // ==== New-user creds modal (same UX as before) ====
    function openUserModal(user, tempPassword = '') {
      // Keep a lightweight object for the modal actions
      const u = {
      id: user.id,
      name: user.name || user.display_name || '',
      email: user.email || '',
      role: user.role || 'Member',
      privileges: [],   // we don’t get exact perms back here; optional to fill later
      password: tempPassword
      };
      // Stash on the dialog for access by handlers
      credsDlg.dataset.user = JSON.stringify(u);
      const nameSpan = document.getElementById('umName');
      if (nameSpan) nameSpan.textContent = u.name || u.email || 'user';
      if (credsDlg?.showModal) credsDlg.showModal();
    }

    function currentModalUser() {
      try { return JSON.parse(credsDlg?.dataset?.user || '{}'); } catch { return null; }
    }

    function downloadCreds(u) {
      const creds = makeCredentialBundle(u);
      const asJson = JSON.stringify(creds, null, 2);
      const name = `${slug(u.name || u.email || 'user')}-credentials.json`;
      ...
      })().catch(() => directDownload(name, asJson, 'application/json'));
      } else {
      directDownload(name, asJson, 'application/json');
      }
    }
    ...
    function emailCreds(u) {
      ...
      `Email: ${u.email}`,
      `Role: ${u.role}`,
      `Temporary password: ${u.password || '(set by admin)'}`,
      '',
    ...
    function directDownload(filename, content, mime='application/octet-stream') {
    ...
    function toast(msg) {
      // minimal toast; use your existing #toast if present
      const holder = document.getElementById('toasts') || document.getElementById('toast');
      if (holder) {
      if (holder.id === 'toast') { holder.textContent = msg; holder.style.display='block'; setTimeout(()=> holder.style.display='none', 2200); return; }
      const t = document.createElement('div');
      t.className = 'toast'; t.textContent = msg;
      holder.appendChild(t); setTimeout(()=> t.remove(), 2200);
      } else {
      alert(msg);
      }
    }
    function safeClose(dlg) { try { dlg?.close?.(); } catch {} }
